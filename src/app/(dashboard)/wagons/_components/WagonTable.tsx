...
function StatusBadge({ status }: { status: TransportStatus }) {
  const map: Record<TransportStatus, { label: string; className: string }> = {
    ...
    unloaded:   { label: "Tushurilgan",  className: "bg-purple-100 text-purple-700" },
...
function calcCubRussia(timber: Timber): number {
  return (timber.thicknessMm / 1000) * (timber.widthMm / 1000) * parseFloat(timber.lengthM) * timber.russiaCount;
}

export default function WagonTable({ transports, onEdit, onDelete }: WagonTableProps) {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 border-b border-slate-200">
            ...
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.wagons.wagonNumber}</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.wagons.from} → {t.wagons.to}</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Status</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.wagons.totalCubRussia}</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.wagons.timbersCount}</th>
            <th className="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.wagons.actions}</th>
          </tr>
        </thead>
        <tbody>
          {transports.map((transport, idx) => {
            ...
            const totalCubRussia = transport.timbers.reduce((sum, timber) => sum + calcCubRussia(timber), 0);
              ...
              <Fragment key={transport.id}>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    >
                      {/* Tahrirlash - har doim (closed ham bo'lsa ko'rish uchun) */}
                      <button
                      ...
                      {isClosed && (
                        <span className="text-xs text-slate-400 px-1">{t.wagons.closed}</span>
                      )}
                    </div>
                  </td>
                </tr>
