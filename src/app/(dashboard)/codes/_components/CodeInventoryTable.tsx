...
export default function CodeInventoryTable({
  codes,
  onDelete,
}: CodeInventoryTableProps) {
  if (codes.length === 0) {
    return (
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200">
            <th className="text-left py-3 px-4 font-semibold text-slate-600 w-12">
              #
            </th>
              ...
              <td className="py-3 px-4">
                <span
                  className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    typeBadgeClasses[code.type] ?? "bg-slate-100 text-slate-600"
                  }`}
                >
